import cv2
import numpy as np
from preprocessing import crop_black_borders, create_nonblack_mask

def analyze_color(image, use_crop=True, use_mask=False):
    """Analyze color characteristics of a mole in the image, with optional cropping and masking."""

    if use_crop:
        image = crop_black_borders(image)

    if use_mask:
        nonblack_mask = create_nonblack_mask(image)
    ...
    if use_mask:
        mask_brown = cv2.bitwise_and(mask_brown, mask_brown, mask=nonblack_mask)
