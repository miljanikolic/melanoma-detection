...
def analyze_color(image):
    ...
    # First close small holes inside the mole
    mask_cleaned = cv2.morphologyEx(mask_brown, cv2.MORPH_CLOSE, kernel)
    
    # Then open to remove small noise outside the mole
    mask_cleaned = cv2.morphologyEx(mask_cleaned, cv2.MORPH_OPEN, kernel)

    # Calculate brown pixel percentage
    brown_pixels = cv2.countNonZero(mask_cleaned)
    total_pixels = image.shape[0] * image.shape[1]
    brown_percentage = (brown_pixels / total_pixels) * 100

    # Visualize the cleaned mask
    result_image = cv2.bitwise_and(image, image, mask=mask_cleaned)
